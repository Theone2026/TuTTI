...
app.post("/api/messages", authenticate, async (req, res) => {
  ...
  if (!buyerId || !text || !productId) {
    return res.status(400).json({ message: "Todos los campos son requeridos." });
  }

  try {
    const sender = await User.findById(senderId); // Obtener datos del remitente
    if (!sender) {
    ...
    const notification = new Notification({
      ...
      message: `Nuevo mensaje sobre el producto: ${product.name}`,
    });
    await notification.save();
    emitNotification(buyerId, notification);

    // Emitir mensaje con datos del remitente
    const messageWithSender = {
      ...newMessage.toObject(),
      senderId: {
        _id: sender._id,
        fullName: sender.fullName,
        profilePicture: sender.profilePicture || "/img/default-avatar.png",
      },
    };
    io.to(buyerId.toString()).emit("newMessage", messageWithSender);

    res.status(201).json({ message: "Mensaje enviado", newMessage: messageWithSender });
  } catch (error) {
